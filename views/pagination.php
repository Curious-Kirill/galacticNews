<?php 
    if (!isset($_GET['p']) || $pageID==1){  // если страница главная, работаем без параметра [p]
      $current_step = 1;
      $p_count = 3; 
    }
    else{
    ...
    for($i=$current_step;$i<=$p_count;$i++):  
      ...
      if($i==$pageID): //выделяем текущую страницу?>  
      ...
      <?php else:
        if($i <= $amountOfPages): //выводим только существующие страницы?>
        <a href="?p=<?=$i?>">   
          <li class="pageNumber ">
            <?=$i?>
          </li>
